...
type Context interface {
	...
	// ShouldBindURI 反序列化 path 参数(如路由路径为 /sys_user_dto/:name)
	// tag: `uri:"xxx"`
	...
	setSessionUserInfo(info session.SessionUserInfo)
...
// ShouldBindURI 反序列化path参数(如路由路径为 /sys_user_dto/:name)
// tag: `uri:"xxx"`
...
func (c *context) SessionUserInfo() session.SessionUserInfo {
	val, ok := c.ctx.Get(_SessionUserInfo)
	if !ok {
		return session.SessionUserInfo{}
	}

	return val.(session.SessionUserInfo)
}

func (c *context) setSessionUserInfo(info session.SessionUserInfo) {
