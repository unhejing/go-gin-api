...
import (
	...
	_ "github.com/unhejing/go-gin-api/docs"
	"github.com/unhejing/go-gin-api/session"
	"github.com/unhejing/go-gin-api/utils/browser"
	"github.com/unhejing/go-gin-api/utils/code"
	"github.com/unhejing/go-gin-api/utils/color"
	"github.com/unhejing/go-gin-api/utils/env"
	"github.com/unhejing/go-gin-api/utils/errors"
	"github.com/unhejing/go-gin-api/utils/trace"
...
// 如：Get /sys_user_dto/:username 这样的路径，因为 username 会有非常多的情况，这样记录指标非常不友好。
...
func WrapAuthHandler(handler func(Context) (sessionUserInfo session.SessionUserInfo, err BusinessError)) HandlerFunc {
	return func(ctx Context) {
...
func New(logger *zap.Logger, options ...Option) (Mux, error) {
	...
	if opt.enableOpenBrowser != "" && env.Active().IsDev() {
		...
		defer func() {
			...
			if ctx.IsAborted() {
				...
				if err := context.abortError(); err != nil { // customer err
					...
					if err.IsAlert() {
						// 发送报警
						logger.Error("警报：traceId", zap.String("traceId", fmt.Sprintf("%s", traceId)))
					}
