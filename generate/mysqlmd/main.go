...
import (
	...
	"github.com/unhejing/go-gin-api/generate/mysqlmd/mysql"
...
func init() {
	...
	table := flag.String("table", "*", "请输入 table\n")
...
func main() {
	...
	for _, table := range tables {
		...
		fmt.Println("  └── file : ", table.Name+"/model.go")
		...
		for _, info := range columnInfo {
			...
			if textType(info.DataType) == "time.Time" { // + "`" + "json:\"" + table.Comment.String + "\"" + "``"
				modelContent += fmt.Sprintf("%s %s `%s` // %s\n", capitalize(info.ColumnName), textType(info.DataType), "json:\""+info.ColumnName+"\" gorm:\"time\"", info.ColumnComment.String)
			} else {
				modelContent += fmt.Sprintf("%s %s `%s` // %s\n", capitalize(info.ColumnName), textType(info.DataType), "json:\""+info.ColumnName+"\"", info.ColumnComment.String)
			}
		}
